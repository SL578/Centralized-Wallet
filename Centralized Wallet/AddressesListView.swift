...
struct AddressListView: View {
    ...
    @ObservedObject var AddressesData: AddressesData
    ...
    var body: some View {
        VStack {
            List {
                ForEach(AddressesData.addresses.filter { $0.type == type }) { address in
                ...
                .onDelete { indexSet in
                    ...
                    let currentTypeAddresses = AddressesData.addresses.filter { $0.type == type }
                    ...
                    for (_, element) in filtered {
                        ...
                        AddressesData.addresses.removeAll { $0.id == element.id }
                            // 每一個位置目前是用 element 表達，要找 element.id 相同的
                            // $0 代表當前的元素，即當前的地址
                            // 若當前地址的 ID 等於 過濾出來地址的 ID, 就刪除當前地址
                    }
                    AddressesData.saveAddresses() //dataManager是物件，saveAddresses() 是該物件有的函數，寫入資料庫
                }
            ...
            HStack {
                ...
                Button("Add") {
                    ...
                    AddressesData.addresses.append(address)   // 寫入資料庫
                    AddressesData.saveAddresses() // 儲存資料庫
